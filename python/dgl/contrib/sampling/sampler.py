...
class NodeFlowSamplerWithDiffBatchSz(object):
    '''Base class that generates NodeFlows from a graph.
    Class properties
    ----------------
    immutable_only : bool
    ...
    def __init__(
            ...
            prefetching_wrapper_class):
        self._g = g
        self._batch_offsets = F.tensor(batch_offsets)
        if self.immutable_only and not g._graph.is_readonly():
            raise NotImplementedError("This loader only support read-only graphs.")
...
class NeighborSamplerWithDiffBatchSz(NodeFlowSamplerWithDiffBatchSz):
    ...
    def __init__(
            ...
            prefetch=False,
            add_self_loop=False):
        super(NeighborSamplerWithDiffBatchSz, self).__init__(
                g, batch_offsets, seed_nodes, shuffle, num_workers * 2 if prefetch else 0,
    ...
    def fetch(self, current_nodeflow_index):
        ...
        nfobjs = _CAPI_NeighborSamplingWithDiffBatchSz(
            self.g._graph,
            self.seed_nodes.todgltensor(),
            current_nodeflow_index, # start batch id
            F.zerocopy_to_dgl_ndarray(self.batch_offsets),        #
